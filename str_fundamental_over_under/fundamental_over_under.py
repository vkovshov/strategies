...
if __name__ == '__main__':
    ...
    # Fetch bucket dates and filter by start and end date
    all_bucket_dates = bucket_dates(s3, s3_bucket_name=s3_bucket, s3_path=S3_DATA_PATH)
    filtered_bucket_dates = [
        date for date in all_bucket_dates
        if start_bucket_date <= date <= end_bucket_date
    ]

    for model_date in filtered_bucket_dates:
